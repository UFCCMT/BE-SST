#!/bin/bash
#Example command:
  #./BE_RUN.sh -c vulcan-proper-symbolic-1024.py -n 1 -v -N 1

#File creates/uses
  #SimulationStarter-vulcan-proper-symbolic-1024.py
    #The file that is created an run by this (BE_RUN.sh) file
  #simulator_build.py
    #Main python file where construciton of the network takes place
    #Called by SS....py file
    #For basic runs, does not need to be edited
      #Only if need to add simulator functionality, e.g. new network
  #vulcan-proper-symbolic-1024.py
    #Machine file where 

#Argtument Handling
while getopts :c:o:i:n:N:d:t:v option
do
        case "${option}"
        in
                c)   SYSTEM_CONFIG=${OPTARG};   #System configuration file
                     SYS_C_PRESENT=1;;
                o)   OUTPUT_FILE=${OPTARG};;    #Output File
                i)   INTERPOLATION=${OPTARG};;  #Which kind of interpolation to use
                n)   NUMBER_RANKS=${OPTARG};    #How many ranks to parallelize on
                     RANK_PRESENT=1;;
                N)   TIMESTEPS=${OPTARG};;      #Timesteps
                d)   DEBUG=${OPTARG};;          #Debug Mide?
                t)   TRACE=${OPTARG};;          #Trace?
...
do
		...
		if [[ $VERBOSE = "TRUE" ]]
		then
			mpirun -np $NUMBER_RANKS sst SimulationStarter-$SYSTEM_CONFIG -v
		else
			mpirun -np $NUMBER_RANKS sst SimulationStarter-$SYSTEM_CONFIG
		fi
	else
		if [[ $VERBOSE = "TRUE" ]]
		then
			mpirun -np 1 sst SimulationStarter-$SYSTEM_CONFIG -v
		else
			mpirun -np 1 sst SimulationStarter-$SYSTEM_CONFIG
		fi
	fi
done

#remove the created sst python config file
#rm SimulationStarter-$SYSTEM_CONFIG
