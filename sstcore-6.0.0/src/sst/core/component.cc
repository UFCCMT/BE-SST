...
std::map<std::tuple<std::string, std::vector<float>>, std::vector<double>>
Component::getGlobalLookupCache()
{
    return Simulation::getSimulation()->getGlobalLookupCache();
}


void 
Component::updateGlobalLookupCache(std::map<std::tuple<std::string, std::vector<float>>, std::vector<double>> cache)
{
    Simulation::getSimulation()->updateGlobalLookupCache(cache);
}
